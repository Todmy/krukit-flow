...
4. **Execute with TDD.** Every task follows red → green → refactor:
   ...
   Iron law: **no production code before a failing test.** If code gets written first, delete it and start from the test. A test that passes the moment you write it is testing existing behavior — fix the test, not the code. The Invariants from context.md MUST NOT break — treat them as untouchable while implementing. A missing interpreter or test runner is part of the task, not a blocker: attempt to install it (system package manager, language toolchain) before any non-executable fallback.
