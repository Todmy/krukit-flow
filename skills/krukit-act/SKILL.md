...
3. **Pick execution mode.** Count the tasks:
   ...
   - **3 or fewer — ask inline vs fresh-context** via the `AskUserQuestion` tool (arrow-key form): first option "Inline (Recommended)" with description "subagent overhead isn't worth it for a small plan." Run whichever the user picks, with the same terminal-step rule.
