...
- **deadline gate** — Layer-0 invariant: on long/search/compute-bound work, write a one-line deadline plan, then climb baseline-first — simplest verifiable deliverable on disk first, improved in place, long rungs backgrounded/bounded. A partial on disk is a safety net against the cutoff, never an exit while budget remains and verification has not passed. Composes with (never overrides) the other invariants.
