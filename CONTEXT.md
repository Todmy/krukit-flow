...
- **capability bar** — categorical minimum model tier per route: `full` requires a Sonnet-class or stronger model, as declared by the harness (never model self-assessment). Below the bar the route caps at `fix`.
