...
### [x] T1: lint route-consistency check
...
### [x] T2: route table v2 in krukit-flow (design §1)
...
### [x] T3: capability self-check + route-log instruction in Stage 0 (design §2+§3)
...
### [x] T4 [P]: flow diet — flow-state as the single tracker
- Modifies: `skills/krukit-flow/SKILL.md`.
...
### [x] T5 [P]: act diet — one confirmation round + Learnings rule
...
### [x] T6 [P]: verify exact-oracle rule + IMPROVE-LOG candidate
...
### [x] T7: README — enumeration, Requirements, P3 compliance
- Modifies: `README.md`.
...
- Renumbering flow steps has non-local blast radius: the §Autonomous mode section cross-references step numbers ("step 8's audit rule") — grep for "step N" across the WHOLE file after any step removal, not just the Process list.
- The lint route-table regex (`| name |` first cell) counts on krukit-flow having exactly one markdown table; if a second table is ever added to that file, the parser needs a section anchor.
